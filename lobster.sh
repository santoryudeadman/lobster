...
[ ! -f "$config_file" ] && printf "player=mpv\nsubs_language=English\nvideo_quality=1080\nbase=https://consumet-api-production-ad05.up.railway.app/movies/flixhq\npreferred_server=vidcloud\n" >"$config_file"
player="$(grep '^player=' "$config_file" | cut -d'=' -f2)" || player="mpv"
base="$(grep '^base=' "$config_file" | cut -d'=' -f2)" || base="https://consumet-api-production-ad05.up.railway.app/movies/flixhq"
